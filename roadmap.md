...
| IDE popup listing queued tasks + current permission state | ✅ Done 2026-07-09 — `extension.js` `checkNotification()` now has explicit cases for `task_approval_pending` (Approve/Discard buttons, runs `askr task approve`/`discard`), `guard_warning` (informational, non-blocking per Phase 3.5 design), and `dangerous_autolaunch_pending` (Approve button, runs `askr launch approve`). |
