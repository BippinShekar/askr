...
| IDE popup listing queued tasks + current permission state | ✅ Done 2026-07-09 — `extension.js` `checkNotification()` has explicit cases for `task_approval_pending` (Approve/Discard buttons, runs `askr task approve`/`discard`) and `guard_warning` (informational, non-blocking per Phase 3.5 design). |
| Gate askr's own autonomous relaunch when the triggering session is dangerous | ❌ **Built 2026-07-09, reverted 2026-07-11 — wrong scope.** `_launch_gate_check()` held every self-continuation relaunch (context/quota/idle/goal triggers) whenever the *same developer's own session* had elevated permissions, treating it as the same risk class as a teammate's queued task. It isn't: a session relaunching itself isn't a new task from anyone else — the developer already has those permissions for their own work. In practice this held every relaunch on every real project, silently (see next row), since broad `allowedTools` is the normal state Phase 3.8 deliberately builds up. Removed entirely — `_start_claude()`/`_open_companion_session()` no longer gate on permission state at all. The cross-dev queue gate above (rows 1–6) is the only place this concern is real, and is unchanged. |
| — regression this caused before revert: held relaunches were completely silent | Found 2026-07-11 — `_notify_launch_held()` wrote Discord + `notification.json` but never called `_speak()`, so a held relaunch produced no voice output at all. Moot now that the gate itself is removed, but noted since it compounded the impact while the gate existed: 25+ hours and 5 sessions with zero trigger-related voice output, discovered via `voice_log.jsonl`. |
...
**Remaining open items post re-audit (updated 2026-07-11):** Phase 3.14 (snapshot-as-architecture — still not built; 3.15 shipped without depending on it by degrading gracefully), Phase 3.9 (behavioral preference persistence), and an unproven real overnight unattended run (Phase 2). IDE popup rendering for `task_approval_pending`/`guard_warning` (Phase 5) — done, see above. Phase 3.15 (smart context injection) — done, see above. Phase 3.13 (persisted rejection tracking) — done, see above. Self-continuation launch gate — built then reverted 2026-07-11, wrong scope, see Phase 5 row above; there is deliberately no gate on a session relaunching its own work regardless of its permission state.
