...
def has_outstanding_subagent(transcript_path: str) -> bool:
    """
    ...
    logical unit of work. Shared by stop.py (gates the spoken "Done." ping)
    and lifecycle.py (gates opening a companion session / auto-launching a
    new one — must not do either while a dispatched subagent is still
    working, same reasoning as not interrupting a turn still generating).
    """
    if not transcript_path or not os.path.exists(transcript_path):
        return False
    try:
...
- decisions: KEEP all existing decisions — never delete one outright, this list is the project's audit trail. But if THIS session's work explicitly reverses, retires, or replaces an existing decision (not just extends it), prefix that existing decision's text with "[SUPERSEDED]" and leave it in place — do not present it unmarked as if it's still current guidance, and do not silently drop it either. A handover that lists a reversed decision next to new ones with no distinction reads as stale/naive to whoever picks it up next. APPEND new decisions from this session normally. CRITICAL: guard blocks from the askr PreToolUse hook ("Guard blocked:", "Write blocked", "awaiting Claude correction") are NOT architectural decisions — they are operational events. Never add guard-inferred or guard-rationalized constraints to the decisions array. Only record decisions the developers explicitly made about product or architecture choices.
